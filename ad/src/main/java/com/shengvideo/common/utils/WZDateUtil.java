 ...
 */
public class WZDateUtil extends org.apache.commons.lang.time.DateUtils {
	...
	public static final SimpleDateFormat LONG_DATE_FORMAT = new SimpleDateFormat(
			"yyyy-MM-dd HH:mm:ss");
	public static final SimpleDateFormat LONG_DATE_FORMAT_SSS = new SimpleDateFormat(
	...
	public static int compareDate(Date date, Date otherDate, int withUnit) {
		...
		case Calendar.MILLISECOND:
			break;
		default:
			throw new IllegalArgumentException("withUnit 单位字段 " + withUnit
					+ " 不合法！！");
		}
	...
	public static Timestamp toSqlTimestamp(String sDate) {
		...
		if (sDate.length() != WZDateUtil.DATE_FORMAT_DATEONLY.length()
				&& sDate.length() != WZDateUtil.DATE_FORMAT_DATETIME.length()) {
			return null;
		}
		return toSqlTimestamp(
				sDate,
				sDate.length() == WZDateUtil.DATE_FORMAT_DATEONLY.length() ? WZDateUtil.DATE_FORMAT_DATEONLY
						: WZDateUtil.DATE_FORMAT_DATETIME);
	...
	public static Timestamp toSqlTimestamp(String sDate, String sFmt) {
		...
		if (sFmt.equals(WZDateUtil.DATE_FORMAT_DATETIME)) {
			temp = sDate.replace('/', '-');
			temp = temp + ".000000000";
		} else if (sFmt.equals(WZDateUtil.DATE_FORMAT_DATEONLY)) {
			temp = sDate.replace('/', '-');
	...
	public static String toSqlTimestampString2(Timestamp dt) {
		...
		String temp = toSqlTimestampString(dt, WZDateUtil.DATE_FORMAT_DATETIME);
	...
	public static String toSqlTimestampString(Timestamp dt, String sFmt) {
		...
		temp = dt.toString();
		if (sFmt.equals(WZDateUtil.DATE_FORMAT_DATETIME) || // "YYYY/MM/DD
				// HH24:MI:SS"
				sFmt.equals(WZDateUtil.DATE_FORMAT_DATEONLY)) { // YYYY/MM/DD
	...
	// 得到当前日期的星期  7.周日
	public static int getWeekSunFirst() {
		Calendar cal = Calendar.getInstance();
		cal.setTime(new Date());
		int w = cal.get(Calendar.DAY_OF_WEEK);
		if (w == 1) {
	...
	public static boolean isBetween(Date now, Date start, Date end, int model) {
		return isBetween(now, start, end, LEFT_OPEN_RIGHT_OPEN, model);
	}
	...
	public static boolean isBetween(Date date, Date start, Date end,
		...
		default: {
			throw new IllegalArgumentException(String.format("日期的比较模式[%d]有误",
					compModel));
		}
		}
		...
		default: {
			throw new IllegalArgumentException(String.format("日期的区间模式[%d]有误",
					interModel));
		}
		}
	}
	...
	public static boolean isOverIntervalLimit(String startDateStr,
		...
		Date endDate = null;
		try {
			startDate = WZDateUtil.parseDate(startDateStr,
					WZDateUtil.DATE_FORMAT_DATEONLY);
			endDate = WZDateUtil.parseDate(endDateStr,
					WZDateUtil.DATE_FORMAT_DATEONLY);
		} catch (ParseException e) {
	...
	public static Date getMinTime(Date dt) {
		Date dt1 = null;
		dt1 = WZDateUtil.getDateByStr(WZDateUtil.formatDate(dt, "yyyy-MM-dd"));
		return dt1;
	}
	...
	public static long getDateDiff(Date date1, Date date2) {
		long diff = (date1.getTime() - date2.getTime()) / (24 * 60 * 60 * 1000) > 0 ? (date1
				.getTime() - date2.getTime()) / (24 * 60 * 60 * 1000)
	...
	public static List<String> getLastDays(int countDay) {
		...
		for (int i = 0; i < countDay; i++) {
			listDate.add(WZDateUtil.getReqDateyyyyMMdd(WZDateUtil.getDate(-i)));
		}
		return listDate;
	}
